...
void SemanticAnalysis::exitFuncFParams(CACTParser::FuncFParamsContext * ctx) {
    for (const auto & oneParam : ctx->funcFParam()) {
        ...
        if (oneParam->ArraySymbol() != nullptr) {
            ...
            if (dt == BOOL || dt == INT) {
                irGen->addCode(new IR_para_w_get(param_var));
            } else if (dt == FLOAT) {
                irGen->addCode(new IR_para_f_get(param_var));
            } else if (dt == DOUBLE) {
                irGen->addCode(new IR_para_d_get(param_var));
            }
        }
    }
}
...
void SemanticAnalysis::exitStmtAssign(CACTParser::StmtAssignContext * ctx) {
    ...
    if (ctx->lVal()->index) {
        if (ctx->lVal()->dataType == INT || ctx->lVal()->dataType == BOOL) {
            irGen->addCode(new IR_w_indexed_copy(ctx->lVal()->result, ctx->exp()->result, ctx->lVal()->index));
        } else if (ctx->lVal()->dataType == FLOAT) {
            irGen->addCode(new IR_f_indexed_copy(ctx->lVal()->result, ctx->exp()->result, ctx->lVal()->index));
        } else if (ctx->lVal()->dataType == DOUBLE) {
            irGen->addCode(new IR_d_indexed_copy(ctx->lVal()->result, ctx->exp()->result, ctx->lVal()->index));
        }
        irGen->endArrOp();
    } else {
        if (ctx->lVal()->dataType == INT || ctx->lVal()->dataType == BOOL) {
            irGen->addCode(new IR_w_Copy(ctx->lVal()->result, ctx->exp()->result));
        } else if (ctx->lVal()->dataType == FLOAT) {
            irGen->addCode(new IR_f_Copy(ctx->lVal()->result, ctx->exp()->result));
        } else if (ctx->lVal()->dataType == DOUBLE) {
            irGen->addCode(new IR_d_Copy(ctx->lVal()->result, ctx->exp()->result));
        }
    }

    if (ctx->flowNext) {
        irGen->addCode(new IR_jump(ctx->flowNext));
    }
}
...
void SemanticAnalysis::exitStmtCtrlIf(CACTParser::StmtCtrlIfContext * ctx) {
    if (ctx->flowEnd) {
        irGen->addCode(new IR_lab(ctx->flowEnd));
    }
}
...
void SemanticAnalysis::enterStmtCtrlIfElse(CACTParser::StmtCtrlIfElseContext * ctx) {    
    ...
    ctx->stmt(0)->codeBefore.push_back(new IR_lab(ctx->cond()->flowTrue));
    ctx->stmt(0)->flowNext = ctx->flowNext;

    ctx->stmt(1)->codeBefore.push_back(new IR_lab(ctx->cond()->flowFalse));
...
void SemanticAnalysis::exitStmtCtrlBreak(CACTParser::StmtCtrlBreakContext * ctx) {
    irGen->addCode(new IR_jump(irGen->getLoopEnd()));
}
...
void SemanticAnalysis::exitStmtCtrlContinue(CACTParser::StmtCtrlContinueContext * ctx) {
    irGen->addCode(new IR_jump(irGen->getLoopBegin()));
}
...
void SemanticAnalysis::exitStmtReturn(CACTParser::StmtReturnContext * ctx) {
    ...
    if (dt == BOOL || dt == INT) {
        irGen->addCode(new IR_w_return(ctx->exp()->result));
    } else if (dt == FLOAT) {
        irGen->addCode(new IR_f_return(ctx->exp()->result));
    } else if (dt == DOUBLE) {
        irGen->addCode(new IR_d_return(ctx->exp()->result));
    }
    irGen->addReturn();
...
void SemanticAnalysis::exitUnaryExpUnaryOp(CACTParser::UnaryExpUnaryOpContext * ctx) {
    ...
    if (unaryOp == "+" || unaryOp == "-") {
        ...
        if (unaryOp == "+") {
            ...
            if (ctx->dataType == INT) {
                irGen->addCode(new IR_int_neg(ctx->result, ctx->unaryExp()->result));
            } else if (ctx->dataType == FLOAT) {
                irGen->addCode(new IR_float_neg(ctx->result, ctx->unaryExp()->result));
            } else if (ctx->dataType == DOUBLE) {
                irGen->addCode(new IR_double_neg(ctx->result, ctx->unaryExp()->result));
            }
        }
        ...
        ctx->result = irGen->newTemp(ctx->dataType);
        irGen->addCode(new IR_bool_not(ctx->result, ctx->unaryExp()->result));
    }
}
...
void SemanticAnalysis::exitMulExpMulExp(CACTParser::MulExpMulExpContext * ctx) {
    ...
    if (mul_op == "*") {
        if (ctx->dataType == INT) {
            irGen->addCode(new IR_int_mul(ctx->result, ctx->mulExp()->result, ctx->unaryExp()->result));
        } else if (ctx->dataType == FLOAT) {
            irGen->addCode(new IR_float_mul(ctx->result, ctx->mulExp()->result, ctx->unaryExp()->result));
        } else if (ctx->dataType == DOUBLE) {
            irGen->addCode(new IR_double_mul(ctx->result, ctx->mulExp()->result, ctx->unaryExp()->result));
        }
    } else if (mul_op == "/") {
        if (ctx->dataType == INT) {
            irGen->addCode(new IR_int_div(ctx->result, ctx->mulExp()->result, ctx->unaryExp()->result));
        } else if (ctx->dataType == FLOAT) {
            irGen->addCode(new IR_float_div(ctx->result, ctx->mulExp()->result, ctx->unaryExp()->result));
        } else if (ctx->dataType == DOUBLE) {
            irGen->addCode(new IR_double_div(ctx->result, ctx->mulExp()->result, ctx->unaryExp()->result));
        }
    } else if (mul_op == "%") {
        if (ctx->dataType == INT) {
            irGen->addCode(new IR_int_mod(ctx->result, ctx->mulExp()->result, ctx->unaryExp()->result));
        }
    }
...
void SemanticAnalysis::exitRelExpAddExp(CACTParser::RelExpAddExpContext * ctx) {
    ...
    if (ctx->flowTrue && ctx->flowFalse) {
        if (ctx->addExp()->dataType == BOOL) {
            irGen->addCode(new IR_greater_than_zero_goto_if(ctx->flowTrue, ctx->addExp()->result));
            irGen->addCode(new IR_jump(ctx->flowFalse));
        } else {
...
void SemanticAnalysis::exitRelExpRelExp(CACTParser::RelExpRelExpContext * ctx) {
    ...
    if (ctx->flowTrue && ctx->flowFalse) {
        if (dt == INT) {
            if (relOp == "<") {
                irGen->addCode(new IR_w_less_than_goto_if(ctx->flowTrue, ctx->relExp()->result, ctx->addExp()->result));
            } else if (relOp == ">") {
                irGen->addCode(new IR_w_greater_than_goto_if(ctx->flowTrue, ctx->relExp()->result, ctx->addExp()->result));
            } else if (relOp == "<=") {
                irGen->addCode(new IR_w_less_equal_than_goto_if(ctx->flowTrue, ctx->relExp()->result, ctx->addExp()->result));
            } else if (relOp == "<=") {
                irGen->addCode(new IR_w_greater_equal_than_goto_if(ctx->flowTrue, ctx->relExp()->result, ctx->addExp()->result));
            }
        } else if (dt == FLOAT) {
            if (relOp == "<") {
                irGen->addCode(new IR_f_less_than_goto_if(ctx->flowTrue, ctx->relExp()->result, ctx->addExp()->result));
            } else if (relOp == ">") {
                irGen->addCode(new IR_f_greater_than_goto_if(ctx->flowTrue, ctx->relExp()->result, ctx->addExp()->result));
            } else if (relOp == "<=") {
                irGen->addCode(new IR_f_less_equal_than_goto_if(ctx->flowTrue, ctx->relExp()->result, ctx->addExp()->result));
            } else if (relOp == "<=") {
                irGen->addCode(new IR_f_greater_equal_than_goto_if(ctx->flowTrue, ctx->relExp()->result, ctx->addExp()->result));
            }
        } else if (dt == DOUBLE) {
            if (relOp == "<") {
                irGen->addCode(new IR_d_less_than_goto_if(ctx->flowTrue, ctx->relExp()->result, ctx->addExp()->result));
            } else if (relOp == ">") {
                irGen->addCode(new IR_d_greater_than_goto_if(ctx->flowTrue, ctx->relExp()->result, ctx->addExp()->result));
            } else if (relOp == "<=") {
                irGen->addCode(new IR_d_less_equal_than_goto_if(ctx->flowTrue, ctx->relExp()->result, ctx->addExp()->result));
            } else if (relOp == "<=") {
                irGen->addCode(new IR_d_greater_equal_than_goto_if(ctx->flowTrue, ctx->relExp()->result, ctx->addExp()->result));
            }
        }
        irGen->addCode(new IR_jump(ctx->flowFalse));
    } else {
...
void SemanticAnalysis::exitRelExpBoolVal(CACTParser::RelExpBoolValContext * ctx) {
    ...
    if (ctx->flowTrue && ctx->flowFalse) {
        irGen->addCode(new IR_greater_than_zero_goto_if(ctx->flowTrue, ctx->boolVal()->result));
        irGen->addCode(new IR_jump(ctx->flowFalse));
    } else {
        // do value operation
        ctx->result = ctx->boolVal()->result;
...
void SemanticAnalysis::exitEqExpEqExp(CACTParser::EqExpEqExpContext * ctx) {
    ...
    if (ctx->flowTrue && ctx->flowFalse) {
        ...
        if (dt == BOOL || dt == INT) {
            irGen->addCode(new IR_w_equal_goto_if(equalWay, ctx->eqExp()->result, ctx->relExp()->result));
        } else if (dt == FLOAT) {
            irGen->addCode(new IR_f_equal_goto_if(equalWay, ctx->eqExp()->result, ctx->relExp()->result));
        } else if (dt == DOUBLE) {
            irGen->addCode(new IR_d_equal_goto_if(equalWay, ctx->eqExp()->result, ctx->relExp()->result));
        }
        irGen->addCode(new IR_jump(notEqualWay));
...
void SemanticAnalysis::enterLAndExpLAndExp(CACTParser::LAndExpLAndExpContext * ctx) {
    ...
    if (ctx->flowTrue && ctx->flowFalse) {
        ...
        ctx->eqExp()->codeBefore.push_back(new IR_lab(ctx->lAndExp()->flowTrue));
        ctx->eqExp()->flowTrue = ctx->flowTrue;
        ctx->eqExp()->flowFalse = ctx->flowFalse;
    } else {
        // do value operation
...
void SemanticAnalysis::enterLOrExpLOrExp(CACTParser::LOrExpLOrExpContext * ctx) {
    if (ctx->flowTrue && ctx->flowFalse) {
        ...
        ctx->lAndExp()->codeBefore.push_back(new IR_lab(ctx->lOrExp()->flowFalse));
        ctx->lAndExp()->flowTrue = ctx->flowTrue;
        ctx->lAndExp()->flowFalse = ctx->flowFalse;
    } else {
        // do value operation
...
void SemanticAnalysis::exitLValIndexed(CACTParser::LValIndexedContext * ctx) {
    ...
    irGen->addCode(new IR_int_mul(ctx->index, ir_cs, ctx->exp()->result));
...
void SemanticAnalysis::exitRValBasic(CACTParser::RValBasicContext * ctx) {
    ...
    if (thisSymbolType == SymbolType::CONST || thisSymbolType == SymbolType::VAR) {
        ...
        if (ctx->dataType == DOUBLE) {
            irGen->addCode(new IR_d_indexed_copyfrom(ctx->result, var, index));
        } else {
            irGen->addCode(new IR_w_indexed_copyfrom(ctx->result, var, index));
        }
    } else {
        throw std::runtime_error(
            std::string("cannot use symbol ") + ctx->Ident()->getText()
...
void SemanticAnalysis::exitRValIndexed(CACTParser::RValIndexedContext * ctx) {
    ...
    IROperand * ir_cs = irGen->newInt(SizeOfDataType(ctx->dataType));
    irGen->addCode(new IR_int_mul(index, ir_cs, ctx->exp()->result));
...
void SemanticAnalysis::exitFuncVal(CACTParser::FuncValContext * ctx) {
    ...
    irGen->addCode(new IR_call(funcLabel));
    if (ctx->dataType == BOOL || ctx->dataType == INT) {
        irGen->addCode(new IR_w_return_get(ctx->result));
    } else if (ctx->dataType == FLOAT) {
        irGen->addCode(new IR_f_return_get(ctx->result));
    } else if (ctx->dataType == DOUBLE) {
        irGen->addCode(new IR_d_return_get(ctx->result));
    }
}
...
void SemanticAnalysis::exitFuncRParams(CACTParser::FuncRParamsContext * ctx) {
    ...
    for (int i = 0; i < paramNum; i++) {
        ...
        if (dt == BOOL || dt == INT) {
            irGen->addCode(new IR_para_w(temp));
        } else if (dt == FLOAT) {
            irGen->addCode(new IR_para_f(temp));
        } else if (dt == DOUBLE) {
            irGen->addCode(new IR_para_d(temp));
        }
    }
}
